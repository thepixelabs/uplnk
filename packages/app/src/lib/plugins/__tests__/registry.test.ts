...
const sampleManifest: PluginManifest = {
  id: 'pylon-plugin-github',
  displayName: 'GitHub',
  description: 'GitHub MCP integration for Pylon',
  version: '1.0.0',
  mcpServer: {
    command: 'npx',
    args: ['-y', 'pylon-plugin-github'],
  },
};

const manifestWithEnv: PluginManifest = {
  id: 'pylon-plugin-linear',
  displayName: 'Linear',
  description: 'Linear issue tracker MCP integration',
  version: '0.2.1',
  mcpServer: {
    command: 'npx',
    args: ['-y', 'pylon-plugin-linear'],
    env: { LINEAR_API_KEY: 'placeholder' },
  },
  requiredEnvVars: ['LINEAR_API_KEY'],
  homepage: 'https://example.com/pylon-plugin-linear',
};
...
describe('PluginRegistry', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'pylon-plugin-test-'));
  ...
  describe('install()', () => {
    it('writes a JSON file to the plugins directory', async () => {
      ...
      const filePath = join(pluginsDir, 'pylon-plugin-github.json');
    ...
    it('written file contains valid JSON matching the manifest', async () => {
      ...
      if (result.success) {
        expect(result.data.id).toBe('pylon-plugin-github');
        expect(result.data.displayName).toBe('GitHub');
        expect(result.data.mcpServer.command).toBe('npx');
        expect(result.data.mcpServer.args).toEqual(['-y', 'pylon-plugin-github']);
      }
    });
    ...
    it('stores env and requiredEnvVars fields when present', async () => {
      ...
      const filePath = join(pluginsDir, 'pylon-plugin-linear.json');
      const raw = JSON.parse(readFileSync(filePath, 'utf-8')) as unknown;
      const result = PluginManifestSchema.safeParse(raw);
      expect(result.success).toBe(true);
      if (result.success) {
        ...
        expect(result.data.homepage).toBe('https://example.com/pylon-plugin-linear');
      }
    });
  });
  ...
  describe('uninstall()', () => {
    it('removes the plugin JSON file', async () => {
      ...
      await registry.uninstall('pylon-plugin-github');
  ...
  describe('list()', () => {
    ...
    it('returns all installed plugin manifests', async () => {
      ...
      expect(ids).toEqual(['pylon-plugin-github', 'pylon-plugin-linear']);
    });
    ...
    it('skips non-JSON files in the plugins directory', async () => {
      ...
      expect(manifests[0]?.id).toBe('pylon-plugin-github');
    });
  ...
  describe('get()', () => {
    it('returns the manifest for an installed plugin', async () => {
      await registry.install(sampleManifest);
      const manifest = registry.get('pylon-plugin-github');
      expect(manifest).toBeDefined();
      expect(manifest?.id).toBe('pylon-plugin-github');
    });
  ...
  describe('toMcpServerConfigs()', () => {
    ...
    it('converts a plugin manifest to a McpServerConfig correctly', async () => {
      ...
      expect(config.id).toBe('plugin:pylon-plugin-github');
      expect(config.name).toBe('GitHub');
      expect(config.type).toBe('stdio');
      expect(config.command).toBe('npx');
      expect(config.args).toEqual(['-y', 'pylon-plugin-github']);
    ...
    it('includes env in McpServerConfig when the plugin manifest has env', async () => {
      ...
      expect(config.id).toBe('plugin:pylon-plugin-linear');
    ...
    it('prefixes the id with "plugin:" to namespace plugin servers', async () => {
      await registry.install(sampleManifest);
      const configs = registry.toMcpServerConfigs();
      expect(configs[0]?.id).toBe('plugin:pylon-plugin-github');
    });
