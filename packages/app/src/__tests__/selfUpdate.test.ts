...
beforeEach(() => {
  vi.clearAllMocks();
  delete process.env['PYLON_NO_UPDATE'];
...
describe('checkForUpdate', () => {
  it('returns null when enabled is false', async () => {
    const result = await checkForUpdate({ packageName: 'pylon-dev', enabled: false });
    expect(result).toBeNull();
  });

  it('returns null when PYLON_NO_UPDATE=1', async () => {
    process.env['PYLON_NO_UPDATE'] = '1';
    const result = await checkForUpdate({ packageName: 'pylon-dev', enabled: true });
    expect(result).toBeNull();
  });
  ...
  it('fetches from npm when no cache exists', async () => {
    ...
    expect(result?.updateCommand).toContain('pylon-dev');
  });
  ...
  it('writes cache after successful fetch', async () => {
    ...
    await checkForUpdate({ packageName: 'pylon-dev', enabled: true });
...
describe('performUpdate', () => {
  ...
  it('calls execFile with npm install -g when no user agent is set', async () => {
    ...
    const lines: string[] = [];
    await performUpdate('pylon-dev', (line) => lines.push(line));

    expect(lines[0]).toContain('npm install -g pylon-dev');
    expect(lines[lines.length - 1]).toContain('Update complete');
    expect(childProcess.execFile).toHaveBeenCalledWith(
      'npm',
      expect.arrayContaining(['install', '-g', 'pylon-dev']),
      expect.anything(),
      expect.any(Function),
    );
  });
  ...
  it('uses pnpm when npm_config_user_agent starts with pnpm', async () => {
    ...
    expect(lines[0]).toContain('pnpm add -g pylon-dev');
    expect(childProcess.execFile).toHaveBeenCalledWith(
      'pnpm',
      expect.arrayContaining(['add', '-g', 'pylon-dev']),
      expect.anything(),
      expect.any(Function),
    );
  });
  ...
  it('uses yarn when npm_config_user_agent starts with yarn', async () => {
    ...
    expect(lines[0]).toContain('yarn global add pylon-dev');
    expect(childProcess.execFile).toHaveBeenCalledWith(
      'yarn',
      expect.arrayContaining(['global', 'add', 'pylon-dev']),
      expect.anything(),
      expect.any(Function),
    );
  });
  ...
  it('propagates execFile error when the update command fails', async () => {
    ...
    await expect(performUpdate('pylon-dev', () => undefined)).rejects.toThrow('Command failed');
  });
});
...
describe('detectPackageManager via updateCommand output', () => {
  ...
  it('updateCommand uses npm by default', async () => {
    ...
    const result = await checkForUpdate({ packageName: 'pylon-dev', enabled: true });
    expect(result?.updateCommand).toBe('npm install -g pylon-dev');
  });
  ...
  it('updateCommand uses pnpm when user agent is pnpm', async () => {
    ...
    const result = await checkForUpdate({ packageName: 'pylon-dev', enabled: true });
    expect(result?.updateCommand).toBe('pnpm add -g pylon-dev');
  });
  ...
  it('updateCommand uses yarn when user agent is yarn', async () => {
    ...
    const result = await checkForUpdate({ packageName: 'pylon-dev', enabled: true });
    expect(result?.updateCommand).toBe('yarn global add pylon-dev');
  });
});
