...
// Check /tmp/uplnk-crash.log after reproducing the bug.
const logCrash = (label: string, err: unknown) => {
  const msg = err instanceof Error ? err.stack ?? err.message : String(err);
  try { appendFileSync('/tmp/uplnk-crash.log', `\n--- ${label} ${new Date().toISOString()} ---\n${msg}\n`); } catch {}
};
...
try {
  ...
  const msg = err instanceof Error ? err.stack ?? err.message : String(err);
  appendFileSync('/tmp/uplnk-crash.log', `\n--- ${stamp} ---\n${msg}\n`);
  process.stderr.write(`\nuplnk crashed — see /tmp/uplnk-crash.log\n`);
  process.exit(1);
}
