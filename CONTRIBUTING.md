...
```bash
pnpm --filter @uplnk/app dev -- --model qwen2.5-coder:7b --theme light
```
...
uplnk/
├── packages/
│   ├── app/          # @uplnk/app — the Ink TUI application and CLI entry point
│   │   ├── bin/
...
- Steps to reproduce
- Contents of `~/.uplnk/crash.log` (if a crash occurred)
